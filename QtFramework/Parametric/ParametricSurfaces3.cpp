...
TriangulatedMesh3*
ParametricSurface3::GenerateImage(GLuint u_div_point_count,
                                  ...
                                  GLenum usage_flag) const
{
  if (_pd.GetRowCount() < 2 || // i.e., if we cannot evaluate the points and
      ...
      u_div_point_count < 2 || // i.e., if the number of u-directional subdivion
                               // points is too small
      v_div_point_count <
        2) // i.e., if the number of v-directional subdivion points is too small
  {
    return 0;
  }
  ...
  for (GLuint i = 0; i < u_div_point_count; ++i) {
    ...
    for (GLuint j = 0; j < v_div_point_count; ++j) {
      ...
      */
      ...
      // the surface normal is obtained as the cross product of the first order
      // partial derivatives
