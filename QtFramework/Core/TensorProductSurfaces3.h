...
public:
  ...
  TensorProductSurface3(GLdouble u_min,
                        GLdouble u_max,
                        GLdouble v_min,
                        GLdouble v_max,
                        GLuint row_count = 4,
  ...
  // F_{n,i}(u) G_{m,j}(v)$,
  //
  // where $n+1$ and $m+1$ denote the row and column counts of the matrix _data,
  // respectively, while
  ...
  // $\left[\mathbf{p}_{i,j}\right]_{i=0,j=0}^{n,m}$ stored by the matrix _data
  // such that interpolation conditions $\mathbf{s}(u_k, v_l) =
