...
inline GLdouble
DCoordinate3::x() const
{
  return _data[0];
}

inline GLdouble
DCoordinate3::y() const
{
  // homework
  return _data[1];
}
...
// change sign
inline const DCoordinate3
DCoordinate3::operator+() const
{
...
{
  return DCoordinate3(
    _data[0] + rhs._data[0], _data[1] + rhs._data[1], _data[2] + rhs._data[2]);
}

// add to *this
inline DCoordinate3&
DCoordinate3::operator+=(const DCoordinate3& rhs) // right hand side
{
  _data[0] += rhs._data[0];
  _data[1] += rhs._data[1];
  _data[2] += rhs._data[2];
...
// cross product, result is stored by *this
inline DCoordinate3&
DCoordinate3::operator^=(const DCoordinate3& rhs) // temp valtozoval
{
  // homework
...
inline const DCoordinate3
DCoordinate3::operator/(const GLdouble& rhs) const
{
  return DCoordinate3(_data[0] / rhs, _data[1] / rhs, _data[2] / rhs);
}
...
// output to stream
inline std::ostream&
operator<<(std::ostream& lhs, const DCoordinate3& rhs)
{
...
// input from stream
inline std::istream&
operator>>(std::istream& lhs, DCoordinate3& rhs)
{
  return lhs >> rhs[0] >> rhs[1] >> rhs[2];
}
}
