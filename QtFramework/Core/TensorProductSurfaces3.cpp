...
GLboolean TensorProductSurface3::RenderData(GLenum render_mode) const{
    if(!_vbo_data)
       ...
       if(render_mode != GL_POINTS && render_mode != GL_LINE_STRIP && render_mode != GL_LINE_LOOP)
       {
           return GL_FALSE;
       }
...
GLboolean TensorProductSurface3::UpdateVertexBufferObjectsOfData(GLenum usage_flag){
    ...
    glBindBuffer(GL_ARRAY_BUFFER, _vbo_data);   //aktivva teszi a buffert
    glBufferData(GL_ARRAY_BUFFER, 2 * _data.GetRowCount() * _data.GetColumnCount() * 3 * sizeof(GLfloat), 0, usage_flag); //helyet foglal
