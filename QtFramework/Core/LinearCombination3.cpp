...
// assignment operator
LinearCombination3::Derivatives&
LinearCombination3::Derivatives::operator=(
  const LinearCombination3::Derivatives& rhs)
{
  if (this != &rhs) {
...
GLvoid
LinearCombination3::Derivatives::LoadNullVectors()
{
...
// special constructor
LinearCombination3::LinearCombination3(GLdouble u_min,
                                       GLdouble u_max,
                                       GLuint data_count,
...
GLvoid
LinearCombination3::DeleteVertexBufferObjectsOfData()
{
  if (_vbo_data) {
...
GLboolean
LinearCombination3::RenderData(GLenum render_mode) const
{
  if (!_vbo_data)
    return GL_FALSE;
...
GLboolean
LinearCombination3::UpdateVertexBufferObjectsOfData(GLenum usage_flag)
{
  ...
  glBindBuffer(GL_ARRAY_BUFFER, _vbo_data);
  glBufferData(
    GL_ARRAY_BUFFER, data_count * 3 * sizeof(GLfloat), 0, _data_usage_flag);
...
GLboolean
LinearCombination3::UpdateDataForInterpolation(
...
GLvoid
LinearCombination3::SetDefinitionDomain(GLdouble u_min, GLdouble u_max)
{
  _u_min = u_min;
  _u_max = u_max;
}
...
GLvoid
LinearCombination3::GetDefinitionDomain(GLdouble& u_min, GLdouble& u_max) const
{
  u_min = _u_min;
  u_max = _u_max;
}
                                  ...
                                  GLenum usage_flag) const
{
  ...
  for (GLuint i = 0; i < div_point_count; ++i) {
    ...
    // cout << result->_derivative.GetRowCount() << " " << d.GetRowCount() <<
    // endl;
