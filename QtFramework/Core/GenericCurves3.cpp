...
GLvoid
GenericCurve3::DeleteVertexBufferObjects()
{
...
GLboolean
GenericCurve3::RenderDerivatives(GLuint order, GLenum render_mode) const
{
...
GLboolean
GenericCurve3::UpdateVertexBufferObjects(GLdouble scale, GLenum usage_flag)
{
  ...
  for (GLuint d = 1; d < _derivative.GetRowCount(); ++d) {
    ...
    glBufferData(
      GL_ARRAY_BUFFER, higher_order_derivative_byte_size, 0, _usage_flag);
...
GLfloat*
GenericCurve3::MapDerivatives(GLuint order, GLenum access_mode) const
{
...
GLboolean
GenericCurve3::UnmapDerivatives(GLuint order) const
{
  if (order >= _derivative.GetRowCount())
    return GL_FALSE;
...
DCoordinate3
GenericCurve3::operator()(GLuint order, GLuint index) const
{
...
DCoordinate3&
GenericCurve3::operator()(GLuint order, GLuint index)
{
  return _derivative(order, index);
}
...
GLboolean
GenericCurve3::SetDerivative(GLuint order,
                             GLuint index,
                             GLdouble x,
                             GLdouble y,
                             GLdouble z)
...
GLboolean
GenericCurve3::SetDerivative(GLuint order, GLuint index, const DCoordinate3& d)
{
  if (order >= _derivative.GetRowCount() ||
      index >= _derivative.GetColumnCount())
    return GL_FALSE;
...
GLboolean
GenericCurve3::GetDerivative(GLuint order,
                             GLuint index,
                             GLdouble& x,
                             GLdouble& y,
                             GLdouble& z) const
...
GLboolean
GenericCurve3::GetDerivative(GLuint order, GLuint index, DCoordinate3& d) const
{
  if (order >= _derivative.GetRowCount() ||
      index >= _derivative.GetColumnCount())
    return GL_FALSE;
...
GLuint
GenericCurve3::GetMaximumOrderOfDerivatives() const
{
  return _derivative.GetRowCount() - 1;
}

GLuint
GenericCurve3::GetPointCount() const
{
  return _derivative.GetColumnCount();
}

GLenum
GenericCurve3::GetUsageFlag() const
{
  return _usage_flag;
}
...
//----------------------------
ostream&
cagd::operator<<(ostream& lhs, const GenericCurve3& rhs)
{
  return lhs << rhs._usage_flag << " " << rhs._derivative << endl;
}

std::istream&
cagd::operator>>(std::istream& lhs, GenericCurve3& rhs)
{
